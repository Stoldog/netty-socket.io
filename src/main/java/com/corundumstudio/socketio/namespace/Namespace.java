...
public class Namespace implements SocketIONamespace {
    ...
    private final ConcurrentMap<String, LinkedHashSet<UUID>> roomClients = PlatformDependent.newConcurrentHashMap();
    private final ConcurrentMap<UUID, LinkedHashSet<String>> clientRooms = PlatformDependent.newConcurrentHashMap();
    ...
    public void addClient(SocketIOClient client) {
        //根据业务逻辑，新增了判断，改动了源代码
        /*SocketIOClient oldClient = allClients.get(client.getSessionId());
        if(oldClient!=null){
            if(client.getHandshakeData().getSingleUrlParam("roomId").equals(oldClient.getHandshakeData().getSingleUrlParam("roomId"))){
                oldClient.leaveRoom(oldClient.getHandshakeData().getSingleUrlParam("roomId"));
            }
    ...
    private <K, V> void join(ConcurrentMap<K, LinkedHashSet<V>> map, K key, V value) {
        LinkedHashSet<V> clients = map.get(key);
        if (clients == null) {
            Set<V> vSet = Collections.newSetFromMap(PlatformDependent.<V, Boolean>newConcurrentHashMap());
            clients = (LinkedHashSet<V>) vSet;
            Set<V> oldClients = map.putIfAbsent(key, clients);
            if (oldClients != null) {
                clients = (LinkedHashSet<V>) oldClients;
            }
        }
        clients.add(value);
    ...
    private <K, V> void leave(ConcurrentMap<K, LinkedHashSet<V>> map, K room, V sessionId) {
        LinkedHashSet<V> clients = map.get(room);
        if (clients == null) {
            return;
        }
    ...
    @Override
    public LinkedHashSet<UUID> getRoomClient(String room) {
